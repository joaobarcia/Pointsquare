Template.registerHelper('mapUnitStateToColor', function(stateValue) {
    if (!Meteor.userId()) {
        ...
        if (stateValue >= 0 && stateValue < 0.4) return "red lighten-2"; // if concept state is low display red book
        else if (stateValue < 0.9) return "orange lighten-2"; // if concept state is intermediate display yellow book
