Template.registerHelper('mapUnitStateToColor', function(stateValue) {
    if (!Meteor.userId()) {
        ...
        if (stateValue >= 0 && stateValue < Session.get("ready threshold")) return "red lighten-2"; // if concept state is low display red book
        else if (stateValue < Session.get("ready threshold")) return "orange lighten-2"; // if concept state is intermediate display yellow book
