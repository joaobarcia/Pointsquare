Template.registerHelper('mapUnitStateToTooltipMsg', function(stateValue) {
    if (!Meteor.userId()) {
        ...
        if (stateValue >= 0 && stateValue < Session.get("readyThreshold")) return "Unit might be too hard for you"; // if concept state is low
        else if (stateValue < Session.get("readyThreshold")) return "Unit might be right for you"; // if concept state is intermediate
