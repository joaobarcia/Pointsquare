Template.registerHelper('mapUnitStateToTooltipMsg', function(stateValue) {
    if (!Meteor.userId()) {
        ...
        if (stateValue >= 0 && stateValue < 0.2) return "Unit is might be too hard for you"; // if concept state is low
        else if (stateValue < 0.8) return "Unit might be right for you"; // if concept state is intermediate
