Template.registerHelper('mapConceptStateToColor', function(stateValue) {
    if (!Meteor.userId()) {
        ...
        if (stateValue >= 0 && stateValue < 0.4) return "grey"; // if concept state is low display grey light bulb
        else if (stateValue < 0.9) return "yellow lighten-1"; // if concept state is intermediate display light yellow light bulb
