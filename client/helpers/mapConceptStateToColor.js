Template.registerHelper('mapConceptStateToColor', function(stateValue) {
    if (!Meteor.userId()) {
        ...
        if (stateValue >= 0 && stateValue < Session.get("ready threshold")) return "grey darken-4"; // if concept state is low display grey light bulb
        else if (stateValue < Session.get("ready threshold")) return "blue-grey lighten-1"; // if concept state is intermediate display light yellow light bulb
