...
Meteor.globalFunctions = {
  ...
  getNeeds: function(nodeId) {
    if (typeof Nodes.findOne(nodeId) !== 'undefined' && Nodes.findOne(nodeId) !== null) {
      ...
      if (node.type == "content") {
        ...
        //console.log("this much I could do");
  ...
  consoleNeeds: function(nodeId) {
    var needs = Meteor.globalFunctions.getNeeds(nodeId);
    for (var set_id in needs.sets){
      console.log("set:")
      set = needs.sets[set_id]
      for (var concept in set){
        name = Nodes.findOne(concept).name;
        console.log(name);
      }
    }
  },
